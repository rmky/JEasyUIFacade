...
class euiButton extends euiAbstractElement
{
    ...
    public function buildHtmlButton()
    {
        ...
        if ($widget->getIconName()) {
            $data_options .= ", iconCls: '" . $this->buildCssIconClass($widget->getIconName()) . "'";
        }
        
        $output = '
				<a id="' . $this->getId() . '" title="' . str_replace('"', '\"', $widget->getCaption()) . '" href="javascript:;" class="easyui-linkbutton" data-options="' . $data_options . '" style="' . $style . '" onclick="' . $this->buildJsFunctionPrefix() . 'click();">
						' . $widget->getCaption() . '
				</a>';
        return $output;
    }
