 ...
 . '/',
    ...
    protected function validate($uploaded_file, $file, $error, $index)
    {
        ...
        if (($max_width || $max_height || $min_width || $min_height) && preg_match($this->options['image_file_types'], $file->name)) {
            ...
            if (@$this->options['image_versions']['']['auto_orient'] && function_exists('exif_read_data') && ($exif = @exif_data_read($uploaded_file)) && (((int) @$exif['Orientation']) >= 5)) {
                $tmp = $img_width;
    ...
    protected function getFileName($file_path, $name, $size, $type, $error, $index, $content_range)
    {
        ...
        return $this->getUniqueFilename($file_path, $this->fixFileExtension($file_path, $name, $size, $type, $error, $index, $content_range), $size, $type, $error, $index, $content_range);
    }
...
;
    ...
    protected function download()
    {
        ...
        if ($redirect_header) {
            return $this->header($redirect_header . ': ' . $this->getDownloadUrl($file_name, $this->getVersionParam(), true));
        }
