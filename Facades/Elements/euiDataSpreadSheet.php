...
{    
    ...
    function buildHtmlHeadTags()
    {
        // handsontable
        $includes = array(
            '<script src="exface/vendor/exface/JEasyUIFacade/Facades/js/handsontable-rulejs/bower_components/handsontable/dist/handsontable.full.min.js"></script>',
            '<link rel="stylesheet" media="screen" href="exface/vendor/exface/JEasyUIFacade/Facades/js/handsontable-rulejs/bower_components/handsontable/dist/handsontable.full.min.css">'
        );
        // formula suppoert
        if ($this->getWidget()->getFormulasEnabled()) {
            $includes[] = '<link rel="stylesheet" media="screen" href="exface/vendor/exface/JEasyUIFacade/Facades/js/handsontable-rulejs/src/handsontable.formula.css">';
            $includes[] = '<script type="text/javascript" src="exface/vendor/exface/JEasyUIFacade/Facades/js/handsontable-rulejs/bower_components/ruleJS/dist/full/ruleJS.all.full.min.js"></script>';
            $includes[] = '<script type="text/javascript" src="exface/vendor/exface/JEasyUIFacade/Facades/js/handsontable-rulejs/src/handsontable.formula.js"></script>';
    ...
    function buildHtml()
    {
        ...
        if ($widget->hasFilters()) {
            foreach ($widget->getFilters() as $fltr) {
                $fltr_html .= $this->getFacade()->buildHtml($fltr);
            }
...
HTML;
    ...
    function buildJs()
    {
        ...
        if ($widget->hasFilters()) {
            foreach ($widget->getFilters() as $fnr => $fltr) {
                $fltr_impl = $this->getFacade()->getElement($fltr);
                $output .= $fltr_impl->buildJs();
                $fltrs[] = '"' . $this->getFacade()->getUrlFilterPrefix() . urlencode($fltr->getAttributeAlias()) . '": ' . $fltr_impl->buildJsValueGetter();
            }
     ...
     *
     * @see \exface\JEasyUIFacade\Facades\Elements\grid::buildJsInitOptionsColumns()
     */
     ...
     *
     * @see \exface\JEasyUIFacade\Facades\Elements\grid::buildJsDataSource()
     */
