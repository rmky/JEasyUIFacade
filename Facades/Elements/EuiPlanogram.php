...
class EuiPlanogram extends EuiDiagram
{
    ...
    public function buildHtml()
    {
        ...
        foreach ($this->getWidget()->getShapes() as $shape) {
            ...
            $button_html .= $this->getFacade()->getElement($shape->getData())->buildHtmlButtons() . "\n";
            $menu_html .= $this->getFacade()->getElement($shape->getData())->buildHtmlContextMenu();
...
<div id="{$this->getId()}_panel" class="easyui-panel" title="{$this->getCaption()}" style="" data-options="fit:true,tools:'#{$this->getId()}_tools',onResize:function(){ if ($('#{$this->getId()} svg').length > 0) { $('#{$this->getId()}').planogram({width: $(this).width(), height: $(this).height()})}}">
	{$this->getFacade()->getElement($this->getWidget()->getDiagramObjectSelectorWidget())->buildHtml()}
	...
	<div id="{$this->getId()}_tools">
		...
		<a href="#" onclick="{$this->buildJsRefresh()};" class="fa fa-refresh" title="{$this->getFacade()->getApp()->getTranslator()->translate('REFRESH')}"></a>
	</div>
...
HTML;
    ...
    public function buildJs()
    {
        ...
        foreach ($widget->getShapes() as $shape) {
            ...
            /* @var $button_element \exface\JEasyUIFacade\Facades\Elements\EuiButton */
            foreach ($shape->getData()->getButtons() as $button) {
                $button_element = $this->getFacade()->getElement($button);
        ...
        if ($widget->getAddRowLinkButtonId()) {
            $add_row_function = $this->getFacade()->getElementByWidgetId($widget->getAddRowLinkButtonId(), $widget->getPage())->buildJsClickFunction();
            ...
            $vm_shelf_oid = $this->getFacade()->getElement($this->getWidget()->getDiagramObjectSelectorWidget())->buildJsValueGetter();
        }
...
JS;
    ...
    public function buildHtmlHeadTags()
    {
        ...
        // $includes[] = '<link rel="stylesheet" media="screen" href="exface/vendor/exface/jEasyUiFacade/Facade/js/planogram/style.css">';
        $includes[] = '<script type="text/javascript" src="exface/vendor/exface/jEasyUiFacade/Facade/js/planogram/planogram.plugin.js"></script>';
        $includes[] = '<script type="text/javascript" src="exface/vendor/exface/jEasyUiFacade/Facade/js/planogram/interact.js"></script>';
        return $includes;
    }
